...
export const DEFAULT_VALUES = {
  ...
  [VARIABLE_KEYS.DISCOUNT_MESSAGE]: "🎉 특별 할인 이벤트 진행중! 지금 바로 확인하세요!",
...
/**
 * A/B Test Variations (예시)
 * Optimizely 프로젝트에서 설정할 수 있는 변형 예시들
 */
export const AB_TEST_VARIATIONS = {
  // 테마 색상 변형
  THEME_COLORS: {
    control: "#007bff", // 파란색 (기본)
    variant_a: "#10b981", // 초록색
    variant_b: "#f59e0b", // 오렌지색
    variant_c: "#8b5cf6", // 보라색
  },
  ...
  // 헤더 메시지 변형
  HEADER_MESSAGES: {
    control: "AI Store에 오신 것을 환영합니다!",
    variant_a: "최고의 상품을 만나보세요!",
    variant_b: "특별한 쇼핑 경험, 지금 시작하세요",
    variant_c: "당신을 위한 특별한 혜택",
  },
  
  // 할인 배너 메시지 변형
  DISCOUNT_MESSAGES: {
    control: "🎉 특별 할인 이벤트 진행중! 지금 바로 확인하세요!",
    variant_a: "💝 한정 특가! 놓치지 마세요!",
    variant_b: "🔥 타임 세일! 지금이 기회!",
    variant_c: "⭐ 회원 전용 특별 할인",
  },
...
export const TEST_DESCRIPTIONS = {
  [FEATURE_FLAGS.UI_THEME_EXPERIMENT]: {
    ...
    hypothesis: "따뜻한 색상(오렌지, 초록)이 차가운 색상(파랑)보다 구매 전환율을 높일 것",
    metrics: [
  ...
  [FEATURE_FLAGS.SHOW_DISCOUNT_BANNER]: {
    ...
    hypothesis: "할인 배너를 표시하면 즉각적인 구매 욕구를 자극하여 전환율이 증가할 것",
    metrics: [
      METRICS_TO_TRACK.ADD_TO_CART_RATE,
  ...
  [FEATURE_FLAGS.PRODUCT_GRID_LAYOUT]: {
    ...
    hypothesis: "2열 그리드가 1열(리스트)이나 3열보다 제품 탐색과 구매 전환에 최적일 것",
    metrics: [
  ...
  [FEATURE_FLAGS.CART_CTA_EXPERIMENT]: {
    ...
    hypothesis: "명확하고 행동 지향적인 CTA 텍스트가 더 높은 전환율을 달성할 것",
    metrics: [
      METRICS_TO_TRACK.ADD_TO_CART_RATE,
      METRICS_TO_TRACK.CHECKOUT_RATE,
    ],
  ...
  [FEATURE_FLAGS.FEATURED_CATEGORIES_EXPERIMENT]: {
    ...
    hypothesis: "인기 카테고리를 우선 노출하면 제품 탐색과 구매 전환이 증가할 것",
    metrics: [
  ...
  [FEATURE_FLAGS.PRODUCT_CARD_STYLE]: {
    ...
    hypothesis: "간결한 디자인이 복잡한 디자인보다 더 나은 사용자 경험을 제공할 것",
    metrics: [
      METRICS_TO_TRACK.PRODUCT_VIEW_COUNT,
      METRICS_TO_TRACK.ADD_TO_CART_RATE,
      METRICS_TO_TRACK.SESSION_DURATION,
